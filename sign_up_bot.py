...
googleTokenPath = 'google_token.json'
...
def update():
    ...
    #Uses the json file with the passwords and other credentials 
    gc = gspread.service_account(filename=googleTokenPath)

    #opens page 1 of the spread sheet
    sh = gc.open(spreadsheetName)

    #gets all the roles from the columns as a list of lists
...
#outputs a message to let you know the bot has logged in
@client.event
async def on_ready():
    print('We have logged in as {0.user}'.format(client))

    #changes what the bot displays as doing
    await client.change_presence(activity = discord.Activity(name="Signing up players", type=1))


#makes it so the bot doesn't respond to itself
@client.event
async def on_message(message):
    ...
    if message.content.startswith('!roles'):
        ...
        #uses the json file with the passwords and other information
        gc = gspread.service_account(filename=googleTokenPath)
        ...
        embed=discord.Embed(title="__**Roles available:**__", color=0x0088ff)
        ...
        if role_number.isdigit() == True:
            ...
            if len(roles)+1 > int(role_number) and int(role_number) > 0:
                ...
                if discord_ids[int(role_number)-1][0] == "Vacant":
                    ...
                    await message.channel.send('That role is already taken!')
            else:
                await message.channel.send("Sorry, this role does not exist...")
        else:
            await message.channel.send("Command incorrectly formatted")
        ...
        if role_number.isdigit() == True:
            ...
            if len(roles)+1 > int(role_number) and int(role_number) > 0:
                ...
                if discord_ids[int(role_number)-1][0] == str(message.author):
                    ...
                    await message.channel.send("That role isn't yours...")
            else:
                await message.channel.send("Sorry, this role does not exist...")
        else:
            await message.channel.send("Command incorrectly formatted")
    ...
    #adds the help command
        ...
        embed.set_thumbnail(url=logoURL)
        embed.add_field(name="!roles", value="Shows you all the roles which are available and their associated numbers", inline=False)
